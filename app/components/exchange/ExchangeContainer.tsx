...
const calculateTotalPrice = (checkOutDate: Date | undefined, checkInDate: Date | undefined, pricePerNight: number) => {
...
const isValidDate = (date: any): date is dayjs.Dayjs | string | number[] => dayjs.isDayjs(date) || typeof date === 'string' || (Array.isArray(date) && date.length === 3);
...
const ExchangeContainer: React.FC<ExchangeContainerProps> = ({
  ...
  const [range, setRange] = useState<Range[]>(
    [{
      startDate: isValidDate(yourTrip.checkInDate) ? dayjs(yourTrip?.checkInDate?.toString())?.toDate() : undefined,
      endDate: isValidDate(yourTrip.checkOutDate) ? dayjs(yourTrip.checkOutDate?.toString())?.toDate() : undefined,
      key: 'selection',
    }]);
  ...
  useEffect(() => {
    ...
    if (range[0]?.startDate && range[0]?.endDate
      && activeStep===0 && dayjs(range[0]?.endDate).isAfter(dayjs(range[0]?.startDate))){
  ...
  useEffect(() => {
    ApartmentForRentApis.getApartmentForRentByAvailableTimeId((yourTrip?.availableTimeId ?? 0).toString())
      .then((res) => {
        setDisableTimes(res?.timeHasBooked?.map((element: any) => {
          const startDate = new Date(element.checkIn);
          const endDate = new Date(element.checkOut);
          return { checkIn: startDate, checkOut: endDate };
        }));
  ...
  useEffect(() => {
    ApartmentForRentApis.getApartmentForRentByAvailableTimeId((exchangeTripData?.availableTimeId ?? 0).toString())
      .catch(err => {
  ...
  const memoizedTotal = useMemo(() => {
    ...
    const exchangeTotalPrice =
      isValidDate(exchangeCheckOutDate) && isValidDate(exchangeCheckInDate)
        ? calculateTotalPrice(dayjs(exchangeCheckOutDate?.toString())?.toDate(), dayjs(exchangeCheckInDate?.toString())?.toDate(), exchangeAvailableTime?.pricePerNight ?? 1)
        : undefined;
    const rangeTotalPrice = calculateTotalPrice(endDate, startDate, yourAvailableTime?.pricePerNight ?? 1);
    return exchangeTotalPrice !== undefined && rangeTotalPrice !== undefined
      ...
      : '----';
  }, [exchangeTripData?.checkOutDate, exchangeTripData?.checkInDate, yourAvailableTime?.pricePerNight, range]);
  ...
  return (<>
    ...
    <Grid className={'w-full h-full'} wrap='nowrap' container spacing={0}>
      <Grid xs={8} className={'border-r-[2px]'}>
        ...
        >
          <div
            className='cursor w-full flex border-b-[1px] dark:border-b-gray-600 py-3 px-2 lg:px-4 justify-between items-center shadow-sm bg-white'>
            <div className='flex w-full gap-3 items-center'>
              ...
              <div className='flex flex-col'>
                <div>
                  {exchanger?.fullName ?? exchanger?.username}
                </div>
              </div>
              ...
              {activeStep !== 3 && activeStep !== -1 && <button type='button'
                ...
                <svg className='w-4 h-4 me-2 -ms-1' aria-hidden='true' focusable='false' data-prefix='fab'
                     data-icon='bitcoin' role='img' xmlns='http://www.w3.org/2000/svg' fill='currentColor'
                     viewBox='0 0 18 20'>
                  <path
          ...
          <div className={'bg-gray-50 lg:px-4'}>
            <div
              className='px-2 py-2 border bg-white dark:border-t-gray-600 flex items-center gap-1 lg:gap-2 w-full rounded-lg'>
          ...
          <TabContext value={tabValue}>
            ...
            <TabPanel className={'p-1 lg:p-2'} value='1'>
              <div className={'w-full h-full overflow-hidden flex flex-col gap-1'}>
                {!isYourAvailableTimeValid && activeStep === 0 && <div
                  className='bg-orange-100 border border-orange-400 text-orange-700 px-4 py-1 rounded relative w-fit'
                  role='alert'>
                  <span className='block sm:inline'>This apartment might be not available anymore.</span>
                </div>}
                <div>
                  <Carousel
                    slide={false}
                    className='relative w-full h-[300px] z-40'
                  >
                    {yourAvailableTime?.coOwner?.property?.propertyImages?.map((image) => (
                      <div key={image.id} className='w-full h-full '>
                        <Image
                          src={image.link ?? 'https://media-cdn.tripadvisor.com/media/photo-s/28/fd/37/ed/pearl-farm-beach-resort.jpg'}
                          alt='destination'
                          fill
                  ...
                  <div className='space-y-4 w-full'>
                    <div className='inline-flex space-x-3'>
                      <Badge
                        name={<div className='flex items-center'>
                          ...
                          <span
                            className='ml-1'>{yourAvailableTime?.coOwner?.property.propertyView.propertyViewName}</span>
                        </div>}
                      />
                      <Badge
                        name={<div className='flex items-center'>
                          ...
                          <span
                            className='ml-1'>{yourAvailableTime?.coOwner?.property.propertyType.propertyTypeName}</span>
                        </div>}
                        color='yellow'
                      />
                    </div>
                    <div className='flex items-center space-x-2'>
                      ...
                      <h2 className='text-lg font-medium capitalize'>
                        <span
                          className='line-clamp-2'>{yourAvailableTime?.coOwner?.property.propertyName}</span>
                      </h2>
                    </div>
                    <div className='inline-grid grid-cols-3 gap-2'>
                      ...
                      <span className='hidden sm:inline-block'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{yourAvailableTime?.coOwner?.property.numberBedsRoom} beds</span>
                      </div>
                      ...
                      <div className='flex items-center space-x-2'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{yourAvailableTime?.coOwner?.property.numberBedsRoom} baths</span>
                      </div>
                      ...
                      <span className='hidden sm:inline-block'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{yourAvailableTime?.coOwner?.property.roomSize} Sq. Fit</span>
                      </div>
                    </div>
                    <div className='flex items-center space-x-2'>
                      <p className='text-gray-600 text-sm ml-2'>
                        <span
                          className='line-clamp-1'>{yourAvailableTime?.coOwner?.property.resort.resortName}</span>
                      </p>
                    </div>
                    <div className='flex w-full justify-between items-end'>
                      ...
                      {yourAvailableTime?.pricePerNight} <Image width={18} height={18}
                                                                src='/images/coin.png' alt='' />/night
                    </span>
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={`grid grid-cols-3 w-full h-full border-y border-gray-300`}
                >
                  <div
                    className={`p-1 border-l border-gray-300`}
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Arrival {activeStep === 0 &&
                      <MdModeEdit
                  ...
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Departure {activeStep === 0 &&
                      <MdModeEdit
                        onClick={() => setOpenDateRange(openDateRange => !openDateRange)}
                        color={'#28ac81'} />}</div>
                    <input
                      ...
                      <DateRange
                        ...
                        maxDate={
                          yourAvailableTime?.endTime && isValidDate(yourAvailableTime.endTime)
                            ? new Date(...(yourAvailableTime.endTime.map((value, index) => index === 1 ? value - 1 : value) as [number, number, number]))
                            : dayjs().add(2, 'month').toDate()
                        }
                        minDate={
                          (yourAvailableTime?.startTime && isValidDate(yourAvailableTime.startTime))
                            ? new Date(...(yourAvailableTime.startTime.map((value, index) => index === 1 ? value - 1 : value) as [number, number, number])) > dayjs().toDate()
                              ? new Date(...(yourAvailableTime.startTime.map((value, index) => index === 1 ? value - 1 : value) as [number, number, number]))
                              : dayjs().toDate()
                            : dayjs().toDate()
                        }
                  ...
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Guests {activeStep === 0 &&
                      <MdModeEdit
                        color={'#28ac81'} />}</div>
                    <input
                      type='number'
                      readOnly
                      value={yourTrip?.numberOfGuest ?? 1}
                      className='border-0 text-sm text-gray-600 focus:outline-0 focus:outline-transparent focus:border-0 focus:border-transparent focus:ring-0 w-full'
                    />
                  </div>
                </div>
                ...
                <div
                  className={`grid grid-cols-2 w-full h-full border-gray-300`}
                >
                  <div
                    className={`p-1 border border-gray-300`}
                  >
                    <div className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Price</div>
                    <input
                      type='text'
                      readOnly
                      value={`${(yourAvailableTime?.pricePerNight && range[0]?.startDate && range[0]?.endDate)
                        ? (yourAvailableTime?.pricePerNight ?? 1) * differenceInDays(range[0]?.endDate, range[0]?.startDate)
                        : '----'
                      }`}
                      className='border-0 text-sm text-gray-600 focus:outline-0 focus:outline-transparent focus:border-0 focus:border-transparent focus:ring-0 w-full'
                    />
                ...
                <div className='flex flex-row justify-between'>
                  {activeStep !== 3 && activeStep !== -1 && activeStep === (yourTripStatus??0) &&
                    <Button onClick={handleNextStep} variant='outlined' className={'max-w-[150px]'} color='warning'>
                      Approve
                    </Button>}
                  {activeStep === 1 && <Button variant='outlined' onClick={handlePreviousStep} className={'max-w-[150px]'} color='error'>
                    Edit
                  </Button>}
                </div>
              </div>
            ...
            <TabPanel className={'p-1 lg:p-2'} value='2'>
              <div className={'w-full h-full overflow-hidden flex flex-col gap-1'}>
                {!isExchangeAvailableTimeValid && activeStep == 0 && <div
                  className='bg-orange-100 border border-orange-400 text-orange-700 px-4 py-1 rounded relative w-fit'
                  role='alert'>
                  <span className='block sm:inline'>This apartment might be not available anymore.</span>
                </div>}
                ...
                <div className='flex-grow p-3 sm:pr-6 flex flex-col items-start'>
                  <div className='space-y-4 w-full'>
                    <div className='inline-flex space-x-3'>
                      <Badge
                        name={<div className='flex items-center'>
                          <i className='text-sm las la-share-alt'></i>
                          <span
                            className='ml-1'>{exchangeAvailableTime?.coOwner?.property.propertyView.propertyViewName}</span>
                        </div>}
                      />
                      <Badge
                        name={<div className='flex items-center'>
                          <i className='text-sm las la-user-friends'></i>
                          <span
                            className='ml-1'>{exchangeAvailableTime?.coOwner?.property.propertyType.propertyTypeName}</span>
                        </div>}
                        color='yellow'
                      />
                    </div>
                    <div className='flex items-center space-x-2'>
                      ...
                      <h2 className='text-lg font-medium capitalize'>
                        <span
                          className='line-clamp-2'>{exchangeAvailableTime?.coOwner?.property.propertyName}</span>
                      </h2>
                    </div>
                    <div className='inline-grid grid-cols-3 gap-2'>
                      <div className='flex items-center space-x-2'>
                      <span className='hidden sm:inline-block'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{exchangeAvailableTime?.coOwner?.property.numberBedsRoom} beds</span>
                      </div>
                      ...
                      <div className='flex items-center space-x-2'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{exchangeAvailableTime?.coOwner?.property.numberBedsRoom} baths</span>
                      </div>
                      ...
                      <span className='hidden sm:inline-block'>
                        ...
                        <span
                          className='text-xs text-neutral-500 dark:text-neutral-400'>{exchangeAvailableTime?.coOwner?.property.roomSize} Sq. Fit</span>
                      </div>
                    </div>
                    <div className='flex items-center space-x-2'>
                      <p className='text-gray-600 text-sm ml-2'>
                        <span
                          className='line-clamp-1'>{exchangeAvailableTime?.coOwner?.property.resort.resortName}</span>
                      </p>
                    </div>
                    <div className='flex w-full justify-between items-end'>
                      <StartRating reviewCount={5} point={3.5} />
                      <span
                        className='flex items-center justify-center px-2.5 py-1.5 border-2 border-yellow-400 rounded-lg leading-none text-sm font-medium text-yellow-400'>
                      {exchangeAvailableTime?.pricePerNight} <Image width={18} height={18}
                                                                    src='/images/coin.png' alt='' />/night
                    </span>
                    </div>
                  </div>
                ...
                >
                  ...
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Arrival
                    </div>
                    <input
                      type='text'
                      readOnly
                      value={isValidDate(exchangeTripData.checkInDate) ? dayjs(exchangeTripData?.checkInDate?.toString())?.format('DD/MM/YYYY') : '-- /-- /----'}
                      className='border-0 text-sm text-gray-600 focus:outline-0 focus:outline-transparent focus:border-0 focus:border-transparent focus:ring-0 w-full'
                    />
                  </div>
                  <div
                  ...
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Departure
                    </div>
                    <input
                      type='text'
                      readOnly
                      value={isValidDate(exchangeTripData.checkOutDate) ? dayjs(exchangeTripData?.checkOutDate?.toString())?.format('DD/MM/YYYY') : '-- /-- /----'}
                      className='border-0 text-sm text-gray-600 focus:outline-0 focus:outline-transparent focus:border-0 focus:border-transparent focus:ring-0 w-full'
                    />
                  </div>
                  <div
                  ...
                  >
                    <div
                      className='px-1 text-sm text-gray-500 flex flex-row justify-between'>Guests
                    </div>
                    <input
                      type='number'
                      readOnly
                ...
                >
                  ...
                  >
                    ...
                    <input
                      type='text'
                      readOnly
                      value={`${(exchangeAvailableTime?.pricePerNight && isValidDate(exchangeTripData.checkOutDate) && isValidDate(exchangeTripData.checkInDate))
                        ? (exchangeAvailableTime?.pricePerNight ?? 1) *
                        differenceInDays(dayjs(exchangeTripData?.checkOutDate?.toString()).toDate(), dayjs(exchangeTripData.checkInDate?.toString()).toDate())
                        : '----'
                      }`}
                      className='border-0 text-sm text-gray-600 focus:outline-0 focus:outline-transparent focus:border-0 focus:border-transparent focus:ring-0 w-full'
                    />
