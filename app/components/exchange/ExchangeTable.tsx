...
const ExchangeTable: React.FC<ExchangeListProps> = ({ initialItems, currentUser }) => {
  ...
  const columns: TableColumnsType<Exchange> = [
    {
      ...
      render: (_, record: Exchange) => (
        <Avatar
          src={`${record?.requestUser?.avatar ?? '/images/placeholder.jpg'}`}
          size='large'
        />
      ),
    },
    {
      ...
      sorter: (a, b) => (a.requestUser?.username ?? '').localeCompare(b?.requestUser?.username ?? ''),
      onFilter: (value: string | number | boolean, record) => record?.requestUser?.username?.startsWith(value?.toString() ?? '')??false,
      filterSearch: true,
    },
    {
      title: 'Status',
      ...
      sortDirections: ['descend', 'ascend'],
      render: (_, record: Exchange) =>
        <Tag color={statusColors[record?.overallStatus] || 'default'}>
    ...
    },
    {
      ...
      sortDirections: ['descend', 'ascend'],
      sorter: (a, b) => (a.lastModifiedOn ? new Date(a.lastModifiedOn).getTime() : 0) - (b.lastModifiedOn ? new Date(b.lastModifiedOn).getTime() : 0),
    },
    {
      title: 'Action', key: 'operation', render: (_, record: Exchange) =>
  ...
  const expandedRowRender = (record: any, expandedIndex: number, indent: number, expanded: boolean) => {
    ...
    for (let i = 0; i < 2; ++i) {
      data.push({
        ...
        arrivalDate: i % 2 === 0
          ? isValidDate(record?.checkInDate) ? format(dayjs(record?.checkInDate?.toString())?.toDate(), 'eeee, MMMM dd, yyyy') : '--/--/----'
          : isValidDate(record?.requestCheckInDate) ? format(dayjs(record?.requestCheckInDate?.toString())?.toDate() || new Date(), 'eeee, MMMM dd, yyyy') : '--/--/----',
        departureDate: i % 2 === 0
          ? isValidDate(record?.checkOutDate) ? format(dayjs(record?.checkOutDate?.toString())?.toDate(), 'eeee, MMMM dd, yyyy') : '--/--/----'
          : isValidDate(record?.requestCheckOutDate) ? format(dayjs(record?.requestCheckOutDate?.toString())?.toDate() || new Date(), 'eeee, MMMM dd, yyyy') : '--/--/----',
      });
    }
