...
const UserMenu: React.FC<UserMenuProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    socket.subscribeNotifications(currentUser).catch((err) => {
  ...
  useEffect(() => {
    if (conversations && conversations.length > 0) {
      ...
      socket.subscribeConversation(conversationIds).catch((err) => {
  ...
  return (
    <>
      ...
      {isOpen ? (
        <div className="absolute rounded-xl shadow-md w-[40vw] md:w-52 border border-gray-300 bg-white overflow-hidden right-20 top-[78px] text-sm z-30">
          <div className="flex flex-col cursor-pointer">
            <Fragment>
              {(() => {
