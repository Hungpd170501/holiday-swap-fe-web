...
const ApartmentDetailBody: React.FC<ApartmentDetailBodyProps> = ({
  ...
  const handleContactOwner = (ownerId: string) => {
    ConversationApis.getContactWithOwner(ownerId).then((res) => {
      res?.conversationId && router.push(`/chat/${res.conversationId}`);
    }).catch((err) => {
        ConversationApis.createCurrentUserConversation(ownerId).then((res) => {
  ...
  return (
    <div className='w-full pb-4 flex flex-col'>
      ...
      <div className='flex flex-col py-8 border-b border-gray-500'>
        ...
        <div
          className='flex flex-col py-3 px-4 items-center justify-start rounded-lg border border-gray-500 w-[20vh] md:flex md:flex-col lg:flex lg:flex-col xl:flex xl:flex-col '>
          <Image src='/images/icons/bed-room.png' alt='icon' width={40} height={25} />
      ...
      <div className='flex flex-col py-8 border-b border-gray-500'>
        ...
        <div onClick={() => apartmentAmenitiesModal.onOpen(apartment)} className='py-4'>
          <button
            className='py-3 px-4 border border-gray-500 rounded-lg w-48 text-center hover:bg-blue-gray-100 hover:cursor-pointer transition-all duration-300 transform active:scale-95'>
            Show all aminities
          </button>
        </div>
      </div>
